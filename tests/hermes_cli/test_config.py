...
class TestSaveAndLoadRoundtrip:
    ...
    def test_load_config_canonicalizes_llama_cpp_provider_alias(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
    ...
    def test_load_config_canonicalizes_local_provider_alias(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            (tmp_path / "config.yaml").write_text(
                yaml.safe_dump(
                    {
    ...
    def test_load_config_migrates_managed_local_custom_endpoint_to_llama_cpp(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            (tmp_path / "config.yaml").write_text(
                yaml.safe_dump(
                    {
    ...
    def test_load_config_strips_removed_llama_cpp_fields(self, tmp_path):
        with patch.dict(os.environ, {"HERMES_HOME": str(tmp_path)}):
            (tmp_path / "config.yaml").write_text(
                yaml.safe_dump(
                    {
